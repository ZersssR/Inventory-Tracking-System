...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("SELECT user_id, email, username, userType, password FROM users WHERE email = ?");
    ...
    if ($user = $result->fetch_assoc()) {
        ...
        if ($user['login_status'] == 1) {
            // Automatically reset login_status to 0 to allow login (kicks existing session)
            $reset_stmt = $conn->prepare("UPDATE users SET login_status = 0 WHERE email = ?");
